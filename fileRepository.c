...
/*
 * $Id: fileRepository.c,v 1.20 2009/11/09 21:03:30 buccella Exp $
 *
...
void
freeBlobIndex(BlobIndex ** bip, int all)
{
  BlobIndex      *bi;
  if (bip == NULL)
    return;
...
static int
indxLocateCase(BlobIndex * bi, const char *key, short ignorecase)
{
  int             loc;
  int             kl = strlen(key);
  bi->next = 0;
...
static int
indxLocate(BlobIndex * bi, const char *key)
{
  // use case-sensitive compare on the key
...
void           *
getFirst(BlobIndex * bi, int *len, char **keyb, size_t * keybl)
{
  char           *buf = NULL;
  bi->next = 0;
...
void           *
getNext(BlobIndex * bi, int *len, char **keyb, size_t * keybl)
{
  char           *buf = NULL;
...
static int
adjust(BlobIndex * bi, int pos, int adj)
{
  int             dp = pos,
      l,
      o,
...
int
addBlob(const char *ns, const char *cls, char *id, void *blob, int len)
{
  int             keyl =
...
int
deleteBlob(const char *ns, const char *cls, const char *id)
{
  int             keyl =
      strlen(ns) + strlen(cls) + strlen(id) + strlen(BASE);
  BlobIndex      *bi;
...
int
existingBlob(const char *ns, const char *cls, const char *id)
{
  int             keyl =
      strlen(ns) + strlen(cls) + strlen(id) + strlen(BASE);
  BlobIndex      *bi;
...
int
existingNameSpace(const char *ns)
{
  int             keyl;
  char           *fn,
                 *p;
